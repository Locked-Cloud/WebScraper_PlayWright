...
class Config:
    ...
    # Download settings - Adjusted for reliability
    MAX_WORKERS = 5  # Reduced from 20 to avoid overwhelming the server
    BATCH_SIZE = 10  # Reduced from 40 to avoid detection
    
    # Delays and timeouts - Increased for reliability
    PAGE_LOAD_DELAY = (2, 5)  # Increased from (0.2, 0.5)
    REQUEST_TIMEOUT = 30     # Increased from 10
    RETRY_DELAY = (3, 7)    # Increased from (1, 3)
    MAX_RETRIES = 2         # Increased from 1
    SELENIUM_TIMEOUT = 30    # Increased from 5
...
class EnhancedCSVImageDownloader:
    ...
    def create_optimized_driver(self):
        """Create a new WebDriver instance with anti-bot detection measures"""
        ...
        for attempt in range(max_attempts):
            try:
                ...
                # Set page load strategy to normal for better reliability
                driver.set_page_load_timeout(Config.SELENIUM_TIMEOUT)
                driver.set_script_timeout(Config.SELENIUM_TIMEOUT)
